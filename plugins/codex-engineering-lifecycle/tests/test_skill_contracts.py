...
class SkillContractTests(unittest.TestCase):
    ...
    def test_main_and_review_explicitly_compose_source_skills(self) -> None:
        ...
        for name in ("feature-lifecycle", "product-workflow-gate", "technical-plan-write"):
            self.assertIn(f"${name}", main)
        review = (SKILLS / "engineering-review" / "SKILL.md").read_text(encoding="utf-8")
    ...
    def test_role_boundaries_and_goal_release_gates_are_documented(self) -> None:
        init = (SKILLS / "engineering-workflow-init" / "SKILL.md").read_text(encoding="utf-8")
        requirements = (SKILLS / "engineering-requirements" / "SKILL.md").read_text(encoding="utf-8")
        main = (SKILLS / "engineering-main" / "SKILL.md").read_text(encoding="utf-8")
        review = (SKILLS / "engineering-review" / "SKILL.md").read_text(encoding="utf-8")
        ...
        self.assertIn("Never check out, commit to, push to, or amend the feature branch", review)
