...
PLUGIN_ROOT = REPOSITORY_ROOT / "plugins" / "codex-claude-engineering-lifecycle"
...
class PluginLayoutTests(unittest.TestCase):
    def test_manifest_marketplace_and_assets_are_consistent(self) -> None:
        ...
        self.assertEqual(manifest["name"], "codex-claude-engineering-lifecycle")
        self.assertEqual(manifest["version"], "0.1.0")
        entry = next(
            item for item in marketplace["plugins"] if item["name"] == manifest["name"]
        )
        self.assertEqual(
            entry["source"]["path"], "./plugins/codex-claude-engineering-lifecycle"
        )
    ...
    def test_user_policy_documents_and_runtime_files_exist(self) -> None:
        for name in (
            ...
            "scripts/claudectl.py",
            "scripts/validate_contracts.py",
            "prompts/claude-frontend.md",
            "prompts/claude-review.md",
        ):
            self.assertTrue((PLUGIN_ROOT / name).is_file(), name)

    def test_claude_prompts_enforce_separate_roles(self) -> None:
        frontend = (PLUGIN_ROOT / "prompts" / "claude-frontend.md").read_text(
            encoding="utf-8"
        )
        review = (PLUGIN_ROOT / "prompts" / "claude-review.md").read_text(
            encoding="utf-8"
        )
