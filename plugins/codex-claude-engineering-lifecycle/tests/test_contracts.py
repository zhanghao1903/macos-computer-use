...
CLAUDECTL_PATH = PLUGIN_ROOT / "scripts" / "claudectl.py"
...
class ContractTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.claudectl = load_module("hybrid_claudectl", CLAUDECTL_PATH)
        ...
        for path in sorted(FIXTURES.glob("*.json")):
            with self.subTest(path=path.name):
                ...
                if ".invalid." in path.name:
                    error_type = (
                        self.workflowctl.WorkflowError
                        if self.validator.CONTRACTS[key][2] == "workflow"
                        else self.claudectl.WorkflowError
                    )
                    with self.assertRaises(error_type):
                        self.validator.validate_runtime_payload(
                            payload,
                            key,
                            self.workflowctl,
                            self.claudectl,
    ...
    def test_validator_cli_reports_fixture_parity(self) -> None:
        ...
        self.assertEqual(report["fixtures"], 26)
